...
// ============================================================
// Helper: return elapsed microseconds between two timespec
// ============================================================
...
int main(int argc, char* argv[])
{
    ...
    error = wasmtime_module_deserialize_file(engine, WASM_FILE, &module);
    ...
    int found = wasmtime_instance_export_get(
        context, &instance,
    ...
    if (error) {
        int status   = 0;
        int is_exit  = wasmtime_error_exit_status(error, &status);
        if (is_exit) {
            exit_code = status;
