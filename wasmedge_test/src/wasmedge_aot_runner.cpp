...
int main(int argc, char* argv[])
{
    ...
    #if 0
    // Tell the WasmEdge runtime that the file to be loaded is a
    // native AOT shared library produced by wasmedge-compile / wasmedgec.
    // WasmEdge_CompilerOutputFormat_Native  → .so / .dylib / .dll
    // WasmEdge_CompilerOutputFormat_Wasm    → universal WASM with AOT section
    //
    // Setting this on the Configure lets the internal Loader skip
    // WASM magic-byte validation and treat the file as a native image.
    WasmEdge_ConfigureCompilerSetOutputFormat(cfg, WasmEdge_CompilerOutputFormat_Native);

    // Optionally force the optimization level that was used during
    // AOT compilation (O3 is the default for wasmedgec).
    WasmEdge_ConfigureCompilerSetOptimizationLevel(
      cfg, WasmEdge_CompilerOptimizationLevel_O3);

    fprintf(stdout, "[INFO] AOT mode: compiler output format set to Native\n");
    #endif
    ...
    WasmEdge_ModuleInstanceInitWASI(
        wasi_module,
        nullptr, 0,   // argv array,  argv count  (no extra arguments)
        nullptr, 0,   // envs array,  env  count  (no environment variables)
        nullptr, 0    // dirs array,  dir  count  (no preopened directories)
    );
    ...
    result = WasmEdge_VMExecute(
        vm,
        start_func,
        nullptr, 0,   // input  parameters (none for _start)
        nullptr, 0    // output return values (none for _start)
    );
    ...
    if (WasmEdge_ResultOK(result)) {
        ...
        if (WasmEdge_ResultGetCategory(result) == WasmEdge_ErrCategory_WASM &&
            ...
            fprintf(stdout, "[INFO] Program exited normally via proc_exit() "
                            "(WASI exit code = %u)\n", wasi_exit);
            exit_code = static_cast<int>(wasi_exit);
        } else {
